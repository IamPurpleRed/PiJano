...
public class Controlls {
	...
	public void do_timer() {
		...
		TimerTask do_controller = new TimerTask() {
			public void run() {
				...
				System.out.println(mushroom + " " + c + " " + old_id.toString() + " " + data + " " + count + " " + if_LB
						+ " " + if_RB);
				controller.poll();//to event//
				eq.getNextEvent(event);
				com = event.getComponent();
						...
						&& if_scored == false)//線中間
				{
					...
					if (target_buttons[0] == '0' && target_buttons[1] == '0' && target_buttons[2] == '0'
							&& target_buttons[3] == '0' && if_have_stuff == true
							&& now_position_y - line_y >= line_height - 6 && now_position_y - line_y <= line_height + 1)//perfect//底牌有東西
					{
						status = "perfect";
						score += 300;
						if_scored = true;
							...
							&& target_buttons[3] == '0' && if_have_stuff == true)//good//底牌有東西
					{
						...
						if_scored = true;
							...
							|| target_buttons[3] != '0' && now_position_y > line_y + line_height + now_height
	...
	public void x_b() {
		if (now_position_y <= line_y + line_height + now_height && now_position_y >= line_y)//在底線範圍內
		{
