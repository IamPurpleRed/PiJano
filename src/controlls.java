...
public class controlls {
	 ...
	 public String status="q";//perfect,good,bad,nothing
		...
		public void run() {
			...
			{ 
						...
						&&id.toString()!="y"&&id.toString()!="z")//trigger進不來
				{
					...
					if(id.toString()=="pov"
					&&target_buttons[0]!='2'&&target_buttons[1]!='2'
						&&target_buttons[2]!='2'&&target_buttons[3]!='2')//非滑塊模式
					{
					 ...
					 else if(id.toString()=="0"||id.toString()=="2"
					 &&target_buttons[0]!='2'&&target_buttons[1]!='2'
						&&target_buttons[2]!='2'&&target_buttons[3]!='2')//2,0//非滑塊模式
					{
						 if(id!=old_id)//沒有連續按
						{
							x_b();
			...
			if(now_position_y>=line_y+20&&now_position_y<=line_y+line_height&&if_scored==false)//滑軌模式//20是畫面誤差
			{
				...
				||target_buttons[2]=='2'||target_buttons[3]=='2')
				{
			...
			if(now_position_y>=line_y&&now_position_y<=line_y+line_height+now_height&&if_scored==false)//線中間,有碰到
				{
							...
							&&if_have_stuff==true)
					{
						...
						if(now_position_y-line_y>=now_height
						&&now_position_y-line_y<=line_height)
						{
							status="perfect";
							score+=300;
							if_scored=true;
					...
					{
						if(target_buttons[0]=='1'||target_buttons[1]=='1'||
					target_buttons[2]=='1'||target_buttons[3]=='1')
						{
							status="bad";
							if_scored=true;
						}
	...
	TimerTask reset=new TimerTask()//danger//每10毫秒做reset
	{
	 ...
	 public void x_b()
	 {
		...
		now_position_y>=line_y&&if_scored==false)//在底線範圍內//包含太快
		{
